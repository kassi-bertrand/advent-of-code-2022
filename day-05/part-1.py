#NOTE:  The "input.txt" file contains the original input
#       Reading and parsing directly "input.txt" is non-trivial
#       So, I handled the input file by:
#       1- Hard coding the initial state of the stacks
#       2- Leaving rearrangement  procedure in a seperate file "procedure.txt"
...
def moveCrates(stacks, crateNumber, srcStack, dstStack):
    ...
    #Move crate(s) one at a time
    for _ in range(crateNumber):
        poppedCrate = stacks[srcStack].pop()
        stacks[dstStack].append(poppedCrate)
...
if __name__ == "__main__":
    ...
    for index in range(1, len(stacks)):
        ...
        print(currList[len(currList) - 1], end="") #Expected TBVFVDZPN